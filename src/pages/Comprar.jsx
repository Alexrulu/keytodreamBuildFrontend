  ...
  return (
    <>
      ...
      <motion.div className="w-full my-3 px-3 flex flex-col gap-5 md:px-10 md:grid md:grid-cols-2 lg:px-20 xl:grid-cols-3"
        ...
        {properties.filter(property => property.type === 1 && property.city.toLowerCase().includes(search)).length === 0 ? (
          ...
          properties.filter(property => property.type === 1 && property.city.toLowerCase().includes(search)).map((property) => (
            <article key={property.id} className="relative overflow-hidden bg-zinc-100 justify-between h-[20vh] group w-full flex gap-1 text-zinc-800 text-sm rounded-xl shadow-lg 
              ...
              <img src={`http://localhost:5000${property.principalImage}`} onClick={() => toggleImgOpen(property.id)}
